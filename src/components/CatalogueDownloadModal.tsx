...
export default function CatalogueDownloadModal({ isOpen, onClose }: CatalogueDownloadModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Store previously active element to return focus to later
      ...
      // Keyboard event handler for ESC and Tab (Focus Trap)
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed inset-0 bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4 z-[9999]"
          onClick={onClose}
        >
          <motion.div
            ref={modalRef}
            ...
            className="bg-white rounded-2xl shadow-2xl w-full max-w-[650px] overflow-hidden border border-slate-100 flex flex-col max-h-[90vh] md:max-h-[85vh] focus:outline-none"
            role="dialog"
            aria-modal="true"
            aria-labelledby="modal-title"
          >
            {/* Header */}
            <div className="bg-corporate-blue text-white px-6 py-5 flex justify-between items-center shrink-0 border-b border-blue-800">
              <div>
                <h3 id="modal-title" className="font-extrabold tracking-wide uppercase text-sm md:text-base flex items-center gap-2">
            ...
            <div className="p-6 overflow-y-auto">
              {submitted ? (
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4 text-sm">
                  ...
                  <div className="bg-blue-50 border border-blue-100 rounded-md p-3.5 text-xs text-corporate-blue leading-relaxed">
                  ...
                  <button
                    type="submit"
                    disabled={isSaving}
                    className="w-full py-3 bg-corporate-blue hover:bg-corporate-blue-hover text-white rounded font-bold text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-2 shadow-xs mt-1 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
